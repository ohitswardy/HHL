...
export function BalanceSheetPage() {
  ...
  const renderAccounts = (accounts: any[]) => accounts?.map((a: any) => (
    <div key={a.code} className="flex justify-between text-sm py-1"><span className="text-[var(--n-text-secondary)] pl-4">{a.code} {a.name}</span><span>{a.balance.toFixed(2)}</span></div>
  ));
    ...
    <div>
      ...
      {data && (
        <Card className="p-6">
          ...
          <p className="text-center text-sm text-[var(--n-text-secondary)] mb-6">Balance Sheet as of {dayjs(data.as_of_date).format('MMMM D, YYYY')}</p>
