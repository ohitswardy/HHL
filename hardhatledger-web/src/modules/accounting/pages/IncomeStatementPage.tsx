...
export function IncomeStatementPage() {
  ...
  return (
    <div>
      ...
      {data && (
        <Card className="p-6">
          <h2 className="text-xl font-bold text-center text-[var(--n-text)] mb-1">HardhatLedger</h2>
          <p className="text-center text-sm text-[var(--n-text-secondary)] mb-6">Income Statement: {dayjs(data.period.start).format('MMM D, YYYY')} - {dayjs(data.period.end).format('MMM D, YYYY')}</p>
          <div className="max-w-md mx-auto space-y-4">
            <div><h3 className="font-semibold text-green-700 border-b pb-1">Revenue</h3>
              {data.revenue_accounts?.map((a: any) => <div key={a.code} className="flex justify-between text-sm py-1"><span className="text-[var(--n-text-secondary)]">{a.code} {a.name}</span><span>{a.amount.toFixed(2)}</span></div>)}
            ...
            <div><h3 className="font-semibold text-red-700 border-b pb-1">Expenses</h3>
              {data.expense_accounts?.map((a: any) => <div key={a.code} className="flex justify-between text-sm py-1"><span className="text-[var(--n-text-secondary)]">{a.code} {a.name}</span><span>{a.amount.toFixed(2)}</span></div>)}
